...
int main()
{
    ...
    std::vector<uint8_t> unit_session_snapshot;
    ...
    if (player.set_faction(cnc::DefinitionId{1U}) != FT_ERR_SUCCESS ||
        ...
        manifest_session.players().set_owner(player_entity, cnc::PlayerId{1U}) != FT_ERR_SUCCESS ||
        manifest_session.bind_factory(player_entity, cnc::DefinitionId{1U}) != FT_ERR_SUCCESS ||
        manifest_session.enqueue_unit_production(cnc::PlayerId{1U}, player_entity,
                                                  cnc::DefinitionId{1U}) !=
            FT_ERR_SUCCESS ||
        ...
        manifest_session.enqueue_unit_production(cnc::PlayerId{1U}, second_factory_entity,
                                                  cnc::DefinitionId{1U}) !=
            FT_ERR_SUCCESS ||
    ...
    if (manifest_session.production().export_snapshot(&completion_snapshot) != FT_ERR_SUCCESS ||
        ...
        manifest_session.bind_unit(cnc::PlayerId{2U}, completion_factory_entity,
                                   cnc::DefinitionId{1U}) != FT_ERR_PERMISSION_DENIED ||
        manifest_session.bind_unit(cnc::PlayerId{1U}, completion_factory_entity,
                                   cnc::DefinitionId{1U}) !=
            FT_ERR_SUCCESS ||
    ...
    if (session.production().enqueue(session_entity, cnc::DefinitionId{1U},
        ...
        return 59;
    cnc::SimulationTick session_saved_power_ready;
    if (session.player_states().find(cnc::PlayerId{1U})->purchase_science(cnc::DefinitionId{1U}) != FT_ERR_SUCCESS ||
        session.player_states().find(cnc::PlayerId{1U})->activate_power(
            cnc::DefinitionId{1U}, session.world().tick(), &session_saved_power_ready) != FT_ERR_SUCCESS)
        return 60;
