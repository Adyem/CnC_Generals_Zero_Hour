...
namespace
{
constexpr ft_size_t header_size = 12U;
...
bool checked_total(ft_size_t world_size, ft_size_t players_size,
                   ft_size_t *total_out) noexcept
{
    ...
    *total_out = with_world + players_size;
    return true;
}
}
                                   ...
                                   std::vector<uint8_t> *bytes_out) noexcept
{
    ...
    if (!checked_total(static_cast<ft_size_t>(world_bytes.size()),
                       static_cast<ft_size_t>(player_bytes.size()), &total_size))
                                   ...
                                   SessionSnapshot *snapshot_out) noexcept
{
    ...
    if (schema != wire_schema_version ||
        !checked_total(static_cast<ft_size_t>(world_size),
                       static_cast<ft_size_t>(players_size), &expected_size) ||
