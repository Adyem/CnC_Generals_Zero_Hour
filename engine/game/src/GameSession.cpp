...
Error GameSession::initialize() noexcept
{
    ...
    error = _factories.initialize(&_catalog);
    if (error != FT_ERR_SUCCESS)
    {
...
Error GameSession::save_snapshot(std::vector<uint8_t> *bytes_out) const noexcept
{
    ...
    if (error != FT_ERR_SUCCESS) return error;
    error = _production.export_snapshot(&snapshot.production);
    if (error != FT_ERR_SUCCESS) return error;
...
Error GameSession::load_snapshot(const uint8_t *bytes, Size byte_count) noexcept
{
    ...
    ProductionQueue projected_production;
    error = projected_production.initialize();
    if (error != FT_ERR_SUCCESS)
    {
        (void)projected_science.shutdown(); (void)projected_powers.shutdown(); (void)projected_generals.shutdown(); (void)projected_players.shutdown(); return error;
    }
    error = projected_production.import_snapshot(snapshot.production);
    if (error != FT_ERR_SUCCESS)
    {
        (void)projected_production.shutdown(); (void)projected_science.shutdown(); (void)projected_powers.shutdown(); (void)projected_generals.shutdown(); (void)projected_players.shutdown(); return error;
    }
    zero_hour::FactoryRegistry projected_factories;
    error = projected_factories.initialize(&_catalog);
    if (error != FT_ERR_SUCCESS)
    {
    ...
    error = projected_factories.import_snapshot(snapshot.factories);
    if (error != FT_ERR_SUCCESS)
    {
...
Error GameSession::bind_unit(PlayerId player, EntityId entity, DefinitionId unit) noexcept
{
    if (_initialized != FT_TRUE ||
        (_phase != Phase::data_ready && _phase != Phase::running))
        return FT_ERR_INVALID_STATE;
    if (!player.is_valid()) return FT_ERR_INVALID_ARGUMENT;
    PlayerId owner;
    const Error owner_error = _players.owner(entity, &owner);
    if (owner_error != FT_ERR_SUCCESS) return owner_error;
    if (owner.value != player.value) return FT_ERR_PERMISSION_DENIED;
    return _units.bind(entity, unit);
}
