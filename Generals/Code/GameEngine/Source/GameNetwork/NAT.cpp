...
void NAT::setConnectionState(Int nodeNumber, NATConnectionState state) {
	...
	Int slotIndex = m_connectionNodes[m_targetNodeNumber].m_slotIndex;
        ...
        if (displaySlot == MAX_SLOTS) {
                DEBUG_ASSERTCRASH(false, ("Didn't find the node number in the slot list"));
                return;
        }
        TheEstablishConnectionsMenu->setPlayerStatus(displaySlot, state);
}
