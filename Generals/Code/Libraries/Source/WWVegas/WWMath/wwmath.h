...
WWINLINE int WWMath::Float_To_Int_Floor (const float& f)
{
	...
	int fractionalMask = (((mantissa << 8) & imask) == 0) ? 1 : 0;
	r = ((r & expsign) ^ (sign)) + (((fractionalMask & (expsign ^ ((a - 1) >> 31)))) & sign);	// if (fabs(value)<1.0) value = 0; copy sign; if (value < 0 && value==(int)(value)) value++;
	return r;
}
