 ...
 *=============================================================================================*/
WWMouseClass::WWMouseClass(Surface * surfaceptr, WWLib::SfmlMessagePump &pump) :
	Blocked(0),
	MouseState(-1),
	IsCaptured(false),
	MouseX(0),
...
{
	MessagePumpListenerId = MessagePump->AddEventListener([this](const sf::Event &event) {
...
{
	if (MessagePump != NULL) {
		sf::Vector2u size = MessagePump->GetWindow().getSize();
		ConfiningRect = Rect(0, 0, static_cast<int>(size.x), static_cast<int>(size.y));
	} else {
		ConfiningRect = Rect(0, 0, 0, 0);
	}
}
...
{
	return static_cast<int>(MouseState.load(std::memory_order_acquire));
}
 ...
 *   03/10/1997 JLB : Created.                                                                 *
 *=============================================================================================*/
void WWMouseClass::Process_Sfml_Event(const sf::Event &event)
{
	switch (event.type) {
...
{
	if (MessagePump != NULL) {
		sf::Vector2i position = sf::Mouse::getPosition(MessagePump->GetWindow());
		x = position.x;
		y = position.y;
	} else {
		x = MouseX;
...
void WWMouseClass::Set_Mouse_XY( int x, int y )
{
	...
	if (MessagePump != NULL) {
		sf::Vector2i position(ConfiningRect.X + x, ConfiningRect.Y + y);
		sf::Mouse::setPosition(position, MessagePump->GetWindow());
	}
